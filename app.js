...
const state = {
  ...
  score: 0, streak: 0, bestStreak: 0, current: null, guess: null,
  revealed: false, currentRoundCommitted: false, roundDeck: [], distances: [],
...
const els = {
  ...
  roundStat:$('roundStat'),scoreStat:$('scoreStat'),streakStat:$('streakStat'),progressFill:$('progressFill'),categoryPill:$('categoryPill'),eraPill:$('eraPill'),factText:$('factText'),hintBox:$('hintBox'),hintBtn:$('hintBtn'),lockBtn:$('lockBtn'),nextBtn:$('nextBtn'),resultPanel:$('resultPanel'),locationName:$('locationName'),pointsEarned:$('pointsEarned'),distanceText:$('distanceText'),contextText:$('contextText'),globeStage:$('globeStage'),statusText:$('statusText'),guessToast:$('guessToast'),endScreen:$('endScreen'),finalScore:$('finalScore'),bestGuess:$('bestGuess'),avgDistance:$('avgDistance'),bestStreak:$('bestStreak'),playAgainBtn:$('playAgainBtn'),soundBtn:$('soundBtn'),endTitle:$('endTitle'),endSubtitle:$('endSubtitle'),zoomInBtn:$('zoomInBtn'),zoomOutBtn:$('zoomOutBtn'),actionDock:$('actionDock'),scoreDock:$('scoreDock'),hudLocation:$('hudLocation'),hudContext:$('hudContext'),hudDistance:$('hudDistance'),hudScore:$('hudScore'),hudRating:$('hudRating'),howToBtn:$('howToBtn'),tutorial:$('tutorial'),tutorialClose:$('tutorialClose'),tutorialGotIt:$('tutorialGotIt'),gameConfigText:$('gameConfigText'),menuBtn:$('menuBtn'),gameMenu:$('gameMenu'),menuClose:$('menuClose'),resumeBtn:$('resumeBtn'),menuHowToBtn:$('menuHowToBtn'),menuSoundBtn:$('menuSoundBtn'),newGameBtn:$('newGameBtn'),confirmNewGame:$('confirmNewGame'),cancelNewGameBtn:$('cancelNewGameBtn'),confirmNewGameBtn:$('confirmNewGameBtn')
};
...
function startGame(){
  ...
  Object.assign(state,{phase:'guessing',resumePhase:null,round:1,totalRounds,score:0,streak:0,bestStreak:0,distances:[],roundDeck,currentRoundCommitted:false,meaningfulProgress:false});
...
function placeGuess(lat,lng){
  ...
  globe.pointsData([{lat,lng,altitude:.012,radius:.45,color:'#65e9ff'}]);
  globe.ringsData([{lat,lng,color:()=> 'rgba(101,233,255,.65)',maxRadius:4,speed:2.2,repeat:900}]);
...
function showEndScreen(){
  ...
  const best=Math.min(...state.distances),avg=state.distances.reduce((a,b)=>a+b,0)/state.distances.length,ratio=state.score/(state.totalRounds*10000);let title='Curious Traveler',subtitle='You found history across the globe.';
...
function closeMenu(){if(state.phase!=='paused')return;state.phase=state.resumePhase||'guessing';state.resumePhase=null;els.globeStage.classList.remove('paused-map');els.gameMenu.classList.remove('show');els.menuBtn.focus()}
...
els.startGameBtn.addEventListener('click',startGame);els.startHowToBtn.addEventListener('click',openTutorial);
els.hintBtn.addEventListener('click',()=>{if(state.phase!=='guessing')return;state.meaningfulProgress=true;els.hintBox.classList.add('show');els.hintBtn.disabled=true;els.hintBtn.textContent='Hint revealed';ping(550,.05)});
...
restorePreferences();
els.gameApp.setAttribute('aria-hidden','true');
