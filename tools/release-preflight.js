...
check('Browser workflow preserves piped command failures', (workflow.match(/set -o pipefail/g) || []).length >= 2);
check('Browser workflow writes a run summary', workflow.includes('$GITHUB_STEP_SUMMARY') && workflow.includes('steps.preflight.outcome') && workflow.includes('steps.browser.outcome'));
