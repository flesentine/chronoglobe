...
check('Browser workflow runs release preflight explicitly', /run:\s*npm run preflight\b/.test(workflow));
check('Browser workflow runs browser-only tests', /run:\s*npm run test:browser\b/.test(workflow));
const workflowPreflightPosition = workflow.indexOf('run: npm run preflight');
const workflowChromiumPosition = workflow.indexOf('run: npx playwright install --with-deps chromium');
const workflowBrowserPosition = workflow.indexOf('run: npm run test:browser');
check('Browser workflow fails fast before Chromium installation', workflowPreflightPosition >= 0 && workflowChromiumPosition > workflowPreflightPosition, `${workflowPreflightPosition} / ${workflowChromiumPosition}`);
